...
class RegexGenTest(unittest.TestCase):
    ...
    def test_build_regex_max_two_children_max_depth_two(self):
        simple_dfa = DFA(self.model, heuristic=Threshold(max_depth=4), store_path=pathlib.Path(self.test_directory.name), root_starter="")
    ...
    def test_simplify_regex_max_two_children_depth_four_center(self):
        simple_dfa = DFA(self.model, heuristic=Threshold(max_depth=4), store_path=pathlib.Path(self.test_directory.name), root_starter="")
        simple_dfa.load_file(pathlib.Path("test/test_simple_nodes_depth_four_middle.gml"))
        simple_dfa.simplify_tree(iterations=3)
        regex = simple_dfa.build_regex()
        self.assertEqual("(a(a[ab]|b(a[ab]|b))|b[ab])", regex)
    ...
    def test_simplify_on_abcd_domain_tree(self):
        dfa = DFA(self.model, heuristic=Threshold(max_depth=4), store_path=pathlib.Path(self.test_directory.name), root_starter="")
